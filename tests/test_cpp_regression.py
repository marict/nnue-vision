...
class TestCppPyTorchRegression:
    ...
    def test_pytorch_vs_cpp_detailed_regression(self, test_model_and_data, device):
        ...
        try:
            ...
            # === Test 1: Sparse Feature Comparison ===
            print("\n=== Testing Sparse Feature Inputs ===")

            # Create deterministic test features
            test_features = [0, 5, 10, 25, 50, 100]  # Known sparse features
            ...
            with torch.no_grad():
                ...
                pytorch_result = model.layer_stacks(pytorch_l0_final, test_layer_stack)

                pytorch_output = pytorch_result.cpu().numpy()[0]

            print(f"PyTorch result: {pytorch_output}")

            # Build C++ regression test
            try:
                result = subprocess.run(
            ...
            if result.returncode != 0:
                pytest.fail(f"C++ regression test failed: {result.stderr}")

            print("C++ regression test output:")
            print(result.stdout)
            ...
            if not cpp_results:
                pytest.fail(
                    f"Could not parse any C++ results from output:\n{result.stdout}"
                )

            # Compare incremental results (layer stack 0)
            if "INCREMENTAL_0" in cpp_results:
                cpp_output = cpp_results["INCREMENTAL_0"]
                print(f"C++ incremental result: {cpp_output}")

                # Calculate errors
                absolute_error = abs(pytorch_output - cpp_output)
                relative_error = absolute_error / (abs(pytorch_output) + 1e-8)

                print(f"Absolute error: {absolute_error}")
                print(f"Relative error: {relative_error}")

                # Regression test criteria - allow for numerical differences
                assert (
                    absolute_error < 1.0
                ), f"Absolute error too large: {absolute_error}"
                assert (
                    relative_error < 0.05
                ), f"Relative error too large: {relative_error} (5%)"

                print("✅ Sparse feature regression test PASSED")
            else:
                print("⚠️ No incremental results found in C++ output")

            # === Test 2: Different Layer Stack Indices ===
            print("\n=== Testing Different Layer Stack Indices ===")
            ...
            for layer_stack_idx in range(model.num_ls_buckets):
                ...
                # C++ (should have been computed in the previous run)
                cpp_key = f"INCREMENTAL_{layer_stack_idx}"
                if cpp_key in cpp_results:
                    cpp_output = cpp_results[cpp_key]

                    absolute_error = abs(pytorch_output - cpp_output)
                    relative_error = absolute_error / (abs(pytorch_output) + 1e-8)

                    print(
                        f"Layer stack {layer_stack_idx}: PyTorch={pytorch_output:.6f}, C++={cpp_output:.6f}, error={absolute_error:.6f}"
                    )

                    assert (
                        absolute_error < 1.0
                    ), f"Layer stack {layer_stack_idx}: absolute error too large: {absolute_error}"
                    assert (
                        relative_error < 0.05
                    ), f"Layer stack {layer_stack_idx}: relative error too large: {relative_error}"

            # === Test 3: Edge Cases ===
            print("\n=== Testing Edge Cases ===")

            # Test empty features
            if "EMPTY" in cpp_results:
                with torch.no_grad():
                    empty_indices = torch.zeros(1, 0, dtype=torch.long, device=device)
                    empty_values = torch.zeros(1, 0, device=device)
                    empty_layer_stack = torch.zeros(1, dtype=torch.long, device=device)
                    ...
                    empty_result = model.layer_stacks(empty_l0_final, empty_layer_stack)

                    pytorch_empty = empty_result.cpu().numpy()[0]
                    cpp_empty = cpp_results["EMPTY"]

                    empty_error = abs(pytorch_empty - cpp_empty)
                    print(
                        f"Empty features: PyTorch={pytorch_empty:.6f}, C++={cpp_empty:.6f}, error={empty_error:.6f}"
                    )
