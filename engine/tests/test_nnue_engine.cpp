...
bool test_conv_layer() {
    ...
    try {
        conv.forward(input.data(), output.data());
        return true;
    } catch (...) {
        return false;
    }
...
bool test_simd_functions() {
    ...
    try {
        ...
        simd::ft_forward_scalar(features, weights.data(), biases.data(), output_scalar.data(), FT_SCALE);
        
        #ifdef __AVX2__
        if (simd::has_avx2()) {
            simd::ft_forward_avx2(features, weights.data(), biases.data(), output_simd.data(), FT_SCALE);
        ...
        #ifdef __ARM_NEON__
        if (simd::has_neon()) {
            simd::ft_forward_neon(features, weights.data(), biases.data(), output_simd.data(), FT_SCALE);
...
void benchmark_simd() {
    ...
    for (int i = 0; i < num_iterations; ++i) {
        simd::ft_forward_scalar(features, weights.data(), biases.data(), output.data(), FT_SCALE);
    }
    ...
    if (simd::has_avx2()) {
        ...
        for (int i = 0; i < num_iterations; ++i) {
            simd::ft_forward_avx2(features, weights.data(), biases.data(), output.data(), FT_SCALE);
        }
    ...
    if (simd::has_neon()) {
        start = std::chrono::high_resolution_clock::now();
        for (int i = 0; i < num_iterations; ++i) {
            simd::ft_forward_neon(features, weights.data(), biases.data(), output.data(), FT_SCALE);
        }
        end = std::chrono::high_resolution_clock::now();
